...
let sServerMasterDataFileName = 'masterdata'
let sVersionColumnName = 'version'
...
let sPhaseSplit = process.argv[process.argv.length - 1].split('_')
let sPhase = sPhaseSplit[0]
...
for (let i = 0; i < aFiles.length; ++i) {
  ...
  for (let j = 0; j < worksheets.length; ++j) {
	...
	let splitList = worksheet.get('ss:Name').split('|');
	
	if (splitList.length <= 1) {
		console.log('ERROR : ' + worksheet.get('ss:Name') + ', not found Define')
	}
      ...
      if (iRowIndex === 0) { // column_access
        ...
        for (let m = 0; m < cells.length; ++m) {
          ...
          if (m === 1 && sDataText !== sVersionColumnName) {
            console.log('ERROR : ' + sTableName + ', Not Exist version - ' + sDataText)
            process.exit(1)
          }
        ...
        for (let m = 0; m < cells.length; ++m) {
          ...
          if (iColumnIdx === 0) {
          ...
          } else if (iColumnIdx === 1) { // version infomation
            if (aColumnTypes[iColumnIdx] !== 'int(11)') {
              console.log('ERROR : ' + sTableName + ', ' + sDataMid + ', ' + aColumnNames[iColumnIdx] + ', invalid version type')
              process.exit(1)
            }
            ...
            let iVersionValue = parseInt(sDataText)

            if (lastVersionInfo[sDataMid]) {
              let iOldVersionValue = lastVersionInfo[sDataMid]

              if (iVersionValue < iOldVersionValue) {
                blSkipSqlInsertRow = true
                break
              } else if (iVersionValue === iOldVersionValue) {
                console.log('ERROR : ' + sTableName + ', ' + sDataMid + ', ' + aColumnNames[iColumnIdx] + ', duplicate version, version : ' + iVersionValue)
                process.exit(1)
              } else {
                console.log('ERROR : ' + sTableName + ', ' + sDataMid + ', ' + aColumnNames[iColumnIdx] + ', invalid version, oldversion : ' + iOldVersionValue + ", nowversion : " + iVersionValue)
                process.exit(1)
              }
            }
            ...
            mstTable[sDataMid][aColumnNames[iColumnIdx]] = parseInt(sDataText)
...
fs.writeFileSync('../js/' + sServerMasterDataFileName + '.json', JSON.stringify(mstDataServer), 'utf8')
fs.writeFileSync('../js/' + sServerMasterDataFileName + '.uncompressed.json', JSON.stringify(mstDataServer, null, 4), 'utf8')
